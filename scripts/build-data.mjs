...
// a Voluntary vs Regrettable workbook) and emits a small, ANONYMIZED data
// file that gets committed and bundled into the static site.
//
// Usage:
...
const vrPath = process.argv[3];

if (!censusPath) {
  console.error('Usage: node scripts/build-data.mjs "<path to census export.xlsx>" ["<path to V&R workbook.xlsx>"]');
  process.exit(1);
}
...
const { roster, terminations } = splitCensusRecords(censusResult.records);
let allRecords = [...roster, ...terminations];
...
if (vrPath) {
  ...
  const joined = joinVrFlags(terminations, vrResult.records);
  allRecords = [...roster, ...joined];
}
